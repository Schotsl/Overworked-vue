<template>
  <form>
    ...
    <select v-model="person" @change="setValue">
      <option v-for="person in persons" :value="person.uuid" :key="person.uuid">
...
export default {
  ...
  methods: {
    ...
    async fetchEntry() {
      ...
      const promises = this.persons.map((person) => {
        return new Promise((resolve, reject) => {
          fetch(
            `${method}://${domain}/${version}/entry?machine=${this.machine}&location=${this.location.uuid}&person=${person.uuid}`
          )
            .then((response) => {
              response
                .json()
    ...
    async addEntry() {
      ...
      await fetch(`${method1}://${domain}/${version}/entry`, {
        headers,
        method,
        body,
      });
